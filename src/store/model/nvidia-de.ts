...
function digitalRiverStockUrl(id: number): string {
	return (
		`https://api.digitalriver.com/v1/shoppers/me/products/${id}/inventory-status?` +
		`&apiKey=${nvidiaApiKey}` +
...
function nvidiaSessionUrl(): string {
	return (
		`https://store.nvidia.com/store/nvidia/SessionToken?format=json&locale=${locale}` +
		`&apiKey=${nvidiaApiKey}` +
		timestampUrlParameter()
	);
}
...
function addToCartUrl(id: number, token: string): string {
	return (
		'https://api.digitalriver.com/v1/shoppers/me/carts/active/line-items?format=json&method=post' +
		`&productId=${id}` +
		`&token=${token}` +
		'&quantity=1' +
		timestampUrlParameter()
...
function generateCartAction(id: number, cardName: string) {
	return async (browser: Browser) => {
		...
		Logger.info(
			`🚀🚀🚀 [nvidia] ${cardName}, starting auto add to cart... 🚀🚀🚀`
		);
		...
		try {
			...
			Logger.info(
				`🚀🚀🚀 [nvidia] ${cardName}, opening checkout page... 🚀🚀🚀`
			);
			Logger.info(checkoutUrl(accessToken));
			await open(checkoutUrl(accessToken));
		} catch {
			Logger.error(
				`✖ [nvidia] ${cardName} could not automatically add to cart, opening page`
			);
...
export const NvidiaDe: Store = {
	links: [
		...
		},
		{
			...
			openCartAction: generateCartAction(
				fe3080Id,
	...
	labels: {
		outOfStock: [
			'product_inventory_out_of_stock',
			'rate limit exceeded',
			'request timeout',
			'resource-not-found'
		]
